		...
		case FSMST_UARTRX:
			if(GetMessage(MSG_UART_RX,(void*)&pRxbuf))
			{
				...
				{//синтаксический анализ команды
					...
					if(pRxbuf[0] == 't' && pRxbuf[1] == 'm' && pRxbuf[2] == '1' && pRxbuf[3] == '2')
          {
                int addr1 = -1, steps1 = -1, addr2 = -1, steps2 = -1;
                // формат: tm12<адрес1:шаги1:адрес2:шаги2>
                sscanf((char*)&(pRxbuf[4]), "<%x:%x:%x:%x>", &addr1, &steps1, &addr2, &steps2);
                
                if(addr1 >= 0 && addr1 <= 0x7FF && steps1 > 0 && 
                   addr2 >= 0 && addr2 <= 0x7FF && steps2 > 0)
                {
                    char cmd1[32], cmd2[32];
                    sprintf(cmd1, "%x:%d", addr1, steps1);
                    sprintf(cmd2, "%x:%d", addr2, steps2);
                    
                    SendMessage(MSG_TM1STRT, (void*)cmd1);
                    SendMessage(MSG_TM2STRT, (void*)cmd2);
                    
                    snprintf((char*)aRecvbuf, IICBUFSZ, "tm12> started MT1@0x%X (%d steps), MT2@0x%X (%d steps)\n", 
                             addr1, steps1, addr2, steps2);
                ...
                {
                    strcpy((char*)aRecvbuf, "Err> tm12: invalid addresses or steps\n");
                    lenfsm = strlen((char*)aRecvbuf);
                    fsmstate = FSMST_UARTTR;
                    break;
                }
