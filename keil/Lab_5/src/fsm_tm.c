  ...
  *wrsymb = rsymb;
  ...
  case FSMST_Q5:
    ...
    if (ctx->base_addr == MT1_BASE_ADDR) {
      SendMessage(MSG_SYNC_MT1_TO_MT2, "STOP");
    } else {
      SendMessage(MSG_SYNC_MT2_TO_MT1, "STOP");
    }
    return -1;
  ...
  if (ctx->base_addr == MT1_BASE_ADDR &&
    ...
    snprintf(sync_data, sizeof(sync_data), "%d:%d", (int)ctx->fsmstate,
             (int)ctx->nextaddr);
    SendMessage(MSG_SYNC_MT1_TO_MT2, sync_data);
    return -2;
  }
  ...
  if (ctx->base_addr == MT2_BASE_ADDR && ctx->nextaddr < MT2_BASE_ADDR) {
    snprintf(sync_data, sizeof(sync_data), "%d:%d", (int)ctx->fsmstate,
             (int)ctx->nextaddr);
    SendMessage(MSG_SYNC_MT2_TO_MT1, sync_data);
    return -2;
  }
...
void ProcessOneTm(struct TM_Context *ctx) {
  ...
  if (GetMessage(MSG_KEYPRESSED, (void *)&pRxbuf)) {
    if (pRxbuf != NULL) {
  ...
  if (ctx->msg_id == MSG_TM2STRT) {
    if (GetMessage(MSG_SYNC_MT1_TO_MT2, (void *)&pRxbuf)) {
  ...
  if (ctx->msg_id == MSG_TM1STRT) {
    if (GetMessage(MSG_SYNC_MT2_TO_MT1, (void *)&pRxbuf)) {
      if (strcmp((char *)pRxbuf, "STOP") == 0) {
        ctx->mtstatectrl = MTCTRL_RXCMD;
        ctx->steptm = -1;
        ctx->stop_requested = 1;
  ...
  case MTCTRL_RXCMD:
    if (GetMessage(ctx->msg_id, (void *)&pRxbuf)) {
      char local_cmd[32];
